...
const SidebarContent = ({ items, title, onClose, openMenu, handleToggle, hoveredIdx, setHoveredIdx }) => {
  ...
  const filteredItems = React.useMemo(() => {
    ...
    return items
      .map((item) => {
        const itemLabelMatches = item.label?.toLowerCase().includes(normalizedQuery);
        if (item.children && item.children.length > 0) {
          const matchingChildren = item.children.filter((child) =>
            child.label?.toLowerCase().includes(normalizedQuery)
          );
          if (itemLabelMatches || matchingChildren.length > 0) {
            return {
              ...item,
              children: itemLabelMatches ? item.children : matchingChildren,
              _forceOpen: true,
            };
          }
          return null;
        }
        return itemLabelMatches ? item : null;
      })
      .filter(Boolean);
  }, [items, normalizedQuery]);

  const totalMatchingPages = React.useMemo(() => {
    if (!normalizedQuery) return items.length;
    return filteredItems.reduce((acc, it) => {
      if (it.children && it.children.length > 0) {
        return acc + it.children.length;
      }
      return acc + 1;
    }, 0);
  }, [filteredItems, normalizedQuery, items.length]);
    ...
    <div className="flex flex-col h-full min-h-0">
      <div className="flex-shrink-0 flex h-16 items-center justify-between px-5 border-b border-white/5 bg-gradient-to-b from-white/[0.02] to-transparent z-10">
        <button
          ...
          className="flex items-center space-x-2.5 text-left rounded-xl hover:bg-white/5 transition-colors cursor-pointer"
      ...
      <div className="px-3 pt-3 pb-1 flex-shrink-0">
        <div className="relative group">
          <svg
            className="absolute left-3 top-1/2 -translate-y-1/2 h-4 w-4 text-gray-400 group-focus-within:text-primary transition-colors pointer-events-none"
            fill="none"
            viewBox="0 0 24 24"
            stroke="currentColor"
            strokeWidth={2}
          ...
          <input
            ...
            placeholder="Search pages & menu..."
            className="w-full pl-9 pr-8 py-2 bg-white/[0.05] hover:bg-white/[0.08] focus:bg-white/[0.12] border border-white/10 focus:border-primary/40 rounded-xl text-xs font-medium text-white placeholder:text-gray-500 transition-all outline-none"
          />
          {searchQuery && (
            <button
              type="button"
              onClick={() => setSearchQuery("")}
              className="absolute right-2.5 top-1/2 -translate-y-1/2 p-0.5 text-gray-400 hover:text-white rounded-md transition-colors"
      ...
      <nav
        ...
        className="mt-2 px-3 space-y-1.5 flex-1 overflow-y-auto overscroll-contain custom-scrollbar-dark min-h-0 pb-6 relative z-20"
        style={{ WebkitOverflowScrolling: 'touch' }}
      >
        <div className="flex items-center justify-between px-3 mb-2">
          <p className="text-[9px] font-black text-gray-500 uppercase tracking-[0.25em]">
            {normalizedQuery ? `Matching Pages (${totalMatchingPages})` : "Core Management"}
          </p>
          {normalizedQuery && (
            <button
              type="button"
        ...
        {filteredItems.length === 0 ? (
          <div className="text-center py-8 px-3 rounded-xl bg-white/[0.02] border border-white/5 mx-1 mt-2">
            <p className="text-xs font-bold text-gray-300">No pages found</p>
            <p className="text-[10px] text-gray-500 mt-1">
              No menu matching &ldquo;{searchQuery}&rdquo;
            </p>
            <button
              type="button"
              onClick={() => setSearchQuery("")}
              className="mt-3 px-3 py-1.5 bg-white/10 hover:bg-white/20 text-white rounded-lg text-xs font-bold transition-all"
            >
              Clear Search
            </button>
          </div>
          ...
          <AnimatePresence>
            {filteredItems.map((item, idx) => (
              <SidebarItem
                ...
                item={item}
                isOpen={Boolean(item._forceOpen || openMenu === item.label)}
